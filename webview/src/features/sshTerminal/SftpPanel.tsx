...
export function SftpPanel({ sftp }: { sftp: SftpActions }) {
	...
	return <aside className="grid min-h-0 min-w-0 grid-rows-[38px_30px_minmax(0,1fr)] border-l border-(--vscode-panel-border,var(--vscode-widget-border)) bg-(--vscode-editor-background) select-none" onClick={() => { setMenu(undefined); setShowMore(false); }}>
		<header className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-1.5 border-b border-(--vscode-panel-border,var(--vscode-widget-border)) py-1 pr-1 pl-2">
			...
			<div className="relative min-w-0"><TextInput aria-label="Remote path" className="h-6.5 pr-7 font-(family-name:--vscode-editor-font-family) text-xs" disabled={sftp.loading} value={pathValue} onFocus={() => setShowFavorites(true)} onChange={event => setPathValue(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && pathValue.trim()) { sftp.list(pathValue.trim()); setShowFavorites(false); } if (event.key === 'Escape') { setPathValue(sftp.sftpPath); setShowFavorites(false); } }} /><IconButton className={`absolute top-px right-px size-6 border-0 ${favorite ? 'text-(--vscode-charts-yellow)' : ''}`} title={favorite ? 'Remove from favorites' : 'Add to favorites'} onClick={event => { event.stopPropagation(); sftp.toggleFavorite(); }}><Star size={14} fill={favorite ? 'currentColor' : 'none'} /></IconButton>{showFavorites && sftp.favorites.length > 0 && <div className="absolute top-7 right-0 left-0 z-20 max-h-52 overflow-auto rounded-[2px] border border-(--vscode-dropdown-border,var(--vscode-panel-border)) bg-(--vscode-dropdown-background) p-1 shadow-[0_2px_8px_var(--vscode-widget-shadow)]">{sftp.favorites.map(path => <button key={path} className="block min-h-7 w-full overflow-hidden border-0 bg-transparent px-2 text-left font-(family-name:--vscode-editor-font-family) text-xs text-ellipsis whitespace-nowrap hover:bg-(--vscode-list-activeSelectionBackground) hover:text-(--vscode-list-activeSelectionForeground)" title={path} onMouseDown={event => event.preventDefault()} onClick={() => { sftp.list(path); setShowFavorites(false); }}>{path}</button>)}</div>}</div>
...
function Menu({ items, className, style }: { items: { icon: React.ReactNode; label: string; danger?: boolean; action: () => void }[]; className: string; style?: React.CSSProperties }) { return <div className={`z-30 min-w-44 rounded-[2px] border border-(--vscode-menu-border,var(--vscode-panel-border)) bg-(--vscode-menu-background) p-1 text-(--vscode-menu-foreground) shadow-[0_2px_8px_var(--vscode-widget-shadow)] ${className}`} style={style}>{items.map(item => <button key={item.label} className={`grid min-h-7 w-full grid-cols-[20px_1fr] items-center border-0 bg-transparent px-2 text-left hover:bg-(--vscode-menu-selectionBackground) hover:text-(--vscode-menu-selectionForeground) ${item.danger ? 'text-(--vscode-errorForeground)' : ''}`} onClick={event => { event.stopPropagation(); item.action(); }}>{item.icon}<span>{item.label}</span></button>)}</div>; }
function Status({ children }: { children: React.ReactNode }) { return <div className="absolute inset-0 flex items-center justify-center bg-[color-mix(in_srgb,var(--vscode-editor-background)_65%,transparent)] p-6 text-center text-xs text-(--vscode-descriptionForeground)">{children}</div>; }
