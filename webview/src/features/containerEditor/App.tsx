...
function ContainerEditDialog({ editor }: { editor: EditorState }) {
	...
	return <div className="fixed inset-0 z-30 grid place-items-center bg-black/45 p-4" onMouseDown={event => { if (event.target === event.currentTarget) editor.closeContainerEditor(); }}>
		<form className="grid h-[min(780px,100%)] w-[min(760px,100%)] min-h-0 grid-rows-[46px_minmax(0,1fr)_54px] overflow-hidden border border-(--vscode-panel-border) bg-(--vscode-editor-background) shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="container-edit-title" onSubmit={event => { event.preventDefault(); editor.recreateContainer(); }}>
			<header className="flex items-center border-b border-(--vscode-panel-border) px-3.5"><h2 id="container-edit-title" className="m-0 text-[13px] font-semibold">Edit and recreate container</h2><IconButton className="ml-auto border-0" type="button" disabled={state.saving} title="Close" onClick={editor.closeContainerEditor}><X size={16} /></IconButton></header>
			...
			<footer className="flex items-center justify-end gap-2 border-t border-(--vscode-panel-border) px-3.5"><button className="h-8.5 border border-(--vscode-button-secondaryBackground) bg-(--vscode-button-secondaryBackground) px-3 text-(--vscode-button-secondaryForeground) hover:bg-(--vscode-button-secondaryHoverBackground) disabled:opacity-55" type="button" disabled={state.saving} onClick={editor.closeContainerEditor}>Cancel</button><PrimaryButton type="submit" disabled={!config || state.loading || state.saving}><RotateCw className={state.saving ? 'animate-spin' : ''} size={15} />{state.saving ? 'Recreating...' : 'Recreate'}</PrimaryButton></footer>
		</form>
	</div>;
}
...
function EditSection({ title, children }: { title: string; children: React.ReactNode }) {
	return <section className="grid gap-3.5 border-t border-(--vscode-panel-border) pt-3.5"><h3 className="m-0 text-xs font-semibold text-(--vscode-descriptionForeground)">{title}</h3>{children}</section>;
}
