...
export function MysqlTablePreviewApp() {
	...
	return <div className="grid h-screen grid-rows-[44px_minmax(0,1fr)_42px] overflow-hidden">
		<header className="flex min-w-0 items-center gap-2 border-b border-(--vscode-panel-border) px-3"><strong className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">{preview.identity.database} / {preview.identity.table}</strong>{data && <span className="text-xs text-(--vscode-descriptionForeground)">{new Intl.NumberFormat().format(data.totalRows)} rows</span>}<span className="ml-auto flex gap-1"><IconButton className="border-0" title="Insert row" onClick={preview.openInsert}><Plus size={16} /></IconButton><IconButton className="border-0" title="Refresh" onClick={preview.refresh}><RefreshCw size={16} /></IconButton></span></header>
...
function RowDialog({ preview }: { preview: PreviewState }) {
	...
	return <Dialog title={dialog.mode === 'insert' ? 'Insert row' : 'Edit row'} onClose={preview.closeDialog} actions={dialog.sql ? <><PrimaryButton className="bg-(--vscode-button-secondaryBackground) text-(--vscode-button-secondaryForeground)" onClick={preview.backToFields}>Back</PrimaryButton><PrimaryButton onClick={preview.confirm}>Execute</PrimaryButton></> : <PrimaryButton onClick={preview.preview}>Review SQL</PrimaryButton>}>
