...
export function Dialog({ title, children, actions, onClose, wide = false }: { title: string; children: ReactNode; actions?: ReactNode; onClose: () => void; wide?: boolean }) {
	return <div className="fixed inset-0 z-30 grid place-items-center bg-black/45 p-5" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
		<section className={`grid max-h-full min-h-0 w-full grid-rows-[42px_minmax(0,1fr)_auto] overflow-hidden rounded-[4px] border border-(--vscode-panel-border,var(--vscode-widget-border)) bg-(--vscode-editor-background) shadow-[0_4px_16px_var(--vscode-widget-shadow)] ${wide ? 'max-w-245' : 'max-w-180'}`} role="dialog" aria-modal="true">
