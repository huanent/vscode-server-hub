...
import type { SqliteColumnInfo, SqlitePreviewRow, SqliteTableFilter, SqliteTablePreviewExtensionMessage, SqliteTableSort } from '../types';

interface TableData {
	columns: string[];
	columnInfo: SqliteColumnInfo[];
	rows: SqlitePreviewRow[];
	canEdit: boolean;
	...
	totalRows: number;
	totalPages: number;
	sort?: SqliteTableSort;
	filters: SqliteTableFilter[];
}

interface RowDialog {
	mode: 'insert' | 'update';
	row?: SqlitePreviewRow;
...
export function useSqliteTablePreview() {
	...
	useEffect(() => {
		const handleMessage = (event: MessageEvent<SqliteTablePreviewExtensionMessage>) => {
	...
	const openUpdate = (row: SqlitePreviewRow) => {
		if (!data) return;
